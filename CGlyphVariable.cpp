...
CGlyphVariable::CGlyphVariable(CGraphicsAdapter* graphicsAdapter, CErrorLog* errorLog, CLocal* local, CShaderBinaryManager* shaderBinaryManager, CTexture* glyphs, int size)
{
	...
	for (int i = 0; i < 94; i++)
	{
		...
		glyphVertices->m_width = (((m_glyphWidth[i + 1] - 1) - m_glyphWidth[i]) + 1.0f);

		glyphVertices->m_vertices[0].m_p.x = m_glyphWidth[i];
		glyphVertices->m_vertices[0].m_p.y = 0.0f;

		glyphVertices->m_vertices[1].m_p.x = (((m_glyphWidth[i + 1] - 1) - m_glyphWidth[i]) + 1.0f) * m_sizeRatio;
		glyphVertices->m_vertices[1].m_p.y = 0.0f;

		glyphVertices->m_vertices[2].m_p.x = (((m_glyphWidth[i + 1] - 1) - m_glyphWidth[i]) + 1.0f) * m_sizeRatio;
		glyphVertices->m_vertices[2].m_p.y = m_glyphHeight * m_sizeRatio;

		glyphVertices->m_vertices[3].m_p.x = m_glyphWidth[i];
		glyphVertices->m_vertices[3].m_p.y = m_glyphHeight * m_sizeRatio;

		glyphVertices->m_vertices[0].m_uv.x = m_glyphWidth[i] / m_glyphs->m_width;
		glyphVertices->m_vertices[0].m_uv.y = 0.0f;

		glyphVertices->m_vertices[1].m_uv.x = (m_glyphWidth[i + 1] - 1) / m_glyphs->m_width;
		glyphVertices->m_vertices[1].m_uv.y = 0.0f;

		glyphVertices->m_vertices[2].m_uv.x = (m_glyphWidth[i + 1] - 1) / m_glyphs->m_width;
		glyphVertices->m_vertices[2].m_uv.y = m_glyphV;

		glyphVertices->m_vertices[3].m_uv.x = m_glyphWidth[i] / m_glyphs->m_width;
