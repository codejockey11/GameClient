	...
	CTextureManager* textureManager, CShaderBinaryManager* shaderBinaryManager, CSoundManager* soundManager, char* name, CShadowMap* shadowMap)
{
	...
	for (int i = 0; i < m_numberOfMaterials; i++)
	{
		...
		m_shader[i].CreateConstantBuffer(0, sizeof(XMFLOAT4X4) * 5 + sizeof(XMFLOAT4) * 2, m_commandList);
		m_shader[i].CreateConstantBuffer(1, 104, m_commandList);
		m_shader[i].CreateConstantBuffer(2, 272, m_commandList);

		m_shader[i].SetTextureCount(3);
		...
		m_overheadShader[i].CreateConstantBuffer(0, sizeof(XMFLOAT4X4) * 5 + sizeof(XMFLOAT4) * 2, m_commandList);
		m_overheadShader[i].CreateConstantBuffer(1, 104, m_commandList);
		m_overheadShader[i].CreateConstantBuffer(2, 272, m_commandList);

		m_overheadShader[i].SetTextureCount(3);
