...
/*
*/
CMap::CMap(CGraphicsAdapter* graphicsAdapter, CErrorLog* errorLog, CLocal* local, CSoundDevice* soundDevice, CWavefrontManager* wavefrontManager, CTextureManager* textureManager,
	...
	int32_t listGroup, CShadowMap* shadowMap, CConstantBuffer* camera, CConstantBuffer* overheadCamera, const char* name, const char* filename)
{
	...
	m_err = fopen_s(&m_fMap, filename, "rb");
	...
	while (!feof(m_fMap))
	{
		...
		case CEntity::Type::COLLECTABLE:
		{
			...
			m_sectors = (CList*)m_visibility->m_collectables->GetElement(3, m_index.m_p.x, m_index.m_p.z, m_index.m_p.y);
		...
		case CEntity::Type::STATICMODEL:
		{
			...
			m_sectors = (CList*)m_visibility->m_statics->GetElement(3, m_index.m_p.x, m_index.m_p.z, m_index.m_p.y);
	...
	for (int32_t i = 0; i < m_numberOfMaterials; i++)
	{
		...
		m_shaderHeap->BindResource(1, m_materialConstantBuffers[i].m_buffer.Get(), &m_materialConstantBuffers[i].m_srvDesc);
		...
		m_overheadShaderHeap->BindResource(1, m_materialConstantBuffers[i].m_buffer.Get(), &m_materialConstantBuffers[i].m_srvDesc);
...
void CMap::DrawGeometry(ComPtr<ID3D12GraphicsCommandList> commandList)
{
	...
	while ((m_node) && (m_node->m_object))
	{
		...
		m_surface->m_indexBuffer->SetCommandList(commandList);
		m_surface->m_indexBuffer->Record();

		m_surface->m_vertexBuffer->SetCommandList(commandList);
		m_surface->m_vertexBuffer->DrawIndexed(m_surface->m_indexBuffer->m_count);
...
void CMap::Record()
{
	...
	while ((m_node) && (m_node->m_object))
	{
		...
		m_surface->m_indexBuffer->SetCommandList(m_commandList);
		m_surface->m_indexBuffer->Record();

		m_surface->m_vertexBuffer->SetCommandList(m_commandList);
		m_surface->m_vertexBuffer->DrawIndexed(m_surface->m_indexBuffer->m_count);
...
{
	m_lightConstantBuffer->Record();

	for (int32_t i = 0; i < m_numberOfMaterials;i++)
	{
		m_materialConstantBuffers[i].Record();
	}
}
