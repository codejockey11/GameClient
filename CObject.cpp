...
void CObject::LoadMeshBuffers(CTextureManager* textureManager)
{
	...
	while (m_meshNode->m_object)
	{
		...
		m_shader->AllocateConstantBuffers(2);
		...
		if (m_useOverhead)
		{
			...
			m_overheadShader->AllocateConstantBuffers(2);

			m_overheadShader->CreateConstantBuffer(0, sizeof(XMFLOAT4X4) * 5 + sizeof(XMFLOAT4) * 2, m_commandList);
			m_overheadShader->CreateConstantBuffer(1, 104, m_commandList);

			m_overheadShader->SetTextureCount(4);
