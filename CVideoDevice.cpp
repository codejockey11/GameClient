...
CVideoDevice::CVideoDevice(CGraphicsAdapter* graphicsAdapter, CErrorLog* errorLog, GUID guid)
{
	...
	HRESULT hr = m_graphicsAdapter->m_device->QueryInterface(IID_PPV_ARGS(&m_videoDevice));

	CVideoDevice::CheckDecoderSupport();
	CVideoDevice::CheckProcessorSupport();

	for (UINT i = 0; i < m_videoDecoderProfiles.ProfileCount; i++)
	{
	...
	hr = m_graphicsAdapter->m_device->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE::D3D12_COMMAND_LIST_TYPE_VIDEO_DECODE, IID_PPV_ARGS(&m_decoderCommandAllocator));
	hr = m_graphicsAdapter->m_device->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE::D3D12_COMMAND_LIST_TYPE_VIDEO_PROCESS, IID_PPV_ARGS(&m_processorCommandAllocator));

	hr = m_graphicsAdapter->m_device->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE::D3D12_COMMAND_LIST_TYPE_VIDEO_DECODE, m_decoderCommandAllocator.Get(), nullptr, IID_PPV_ARGS(&m_decoderCommandList));
	hr = m_graphicsAdapter->m_device->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE::D3D12_COMMAND_LIST_TYPE_VIDEO_PROCESS, m_processorCommandAllocator.Get(), nullptr, IID_PPV_ARGS(&m_processorCommandList));
	...
	hr = m_graphicsAdapter->m_device->CreateCommittedResource(&m_textureHeapProperties,
	...
	hr = m_graphicsAdapter->m_device->CreateCommittedResource(&m_mpegBufferHeapProperties,
		D3D12_HEAP_FLAGS::D3D12_HEAP_FLAG_NONE,
		&m_mpegBufferDesc,
		D3D12_RESOURCE_STATES::D3D12_RESOURCE_STATE_GENERIC_READ,
		nullptr,
		IID_PPV_ARGS(&m_mpegBuffer));
...
CVideoDevice::~CVideoDevice()
{
	for (UINT i = 0; i < m_videoDecoderProfiles.ProfileCount; i++)
	{
		delete[] m_videoDecoderFormats[i].pOutputFormats;
	}

	delete[] m_videoDecoderFormats;
	delete[] m_videoDecoderFormatCounts;
	delete[] m_videoDecoderConfigurations;
	delete[] m_guids;
...
void CVideoDevice::CheckDecoderSupport()
{
	...
	HRESULT hr = m_videoDevice->CheckFeatureSupport(D3D12_FEATURE_VIDEO::D3D12_FEATURE_VIDEO_DECODE_PROFILE_COUNT, (void*)&m_videoDecoderProfileCount, sizeof(m_videoDecoderProfileCount));
	...
	hr = m_videoDevice->CheckFeatureSupport(D3D12_FEATURE_VIDEO::D3D12_FEATURE_VIDEO_DECODE_PROFILES, (void*)&m_videoDecoderProfiles, sizeof(m_videoDecoderProfiles));
	...
	for (UINT i = 0; i < m_videoDecoderProfiles.ProfileCount; i++)
	{
		...
		hr = m_videoDevice->CheckFeatureSupport(D3D12_FEATURE_VIDEO::D3D12_FEATURE_VIDEO_DECODE_FORMAT_COUNT, (void*)&m_videoDecoderFormatCounts[i], sizeof(m_videoDecoderFormatCounts[i]));
		...
		hr = m_videoDevice->CheckFeatureSupport(D3D12_FEATURE_VIDEO::D3D12_FEATURE_VIDEO_DECODE_FORMATS, (void*)&m_videoDecoderFormats[i], sizeof(m_videoDecoderFormats[i]));

		char* gname = CVideoDevice::GetProfileName(m_guids[i]);

		m_errorLog->WriteError(true, "Profile:%i %s\n", i, gname);

		for (UINT j = 0; j < m_videoDecoderFormats[i].FormatCount; j++)
		{
			char* n = m_graphicsAdapter->GetDXGIFormatName(m_DXGIFormats[j]);
...
{
	HRESULT hr = m_videoDevice->CheckFeatureSupport(D3D12_FEATURE_VIDEO::D3D12_FEATURE_VIDEO_PROCESS_MAX_INPUT_STREAMS, (void*)&m_videoProcessMaxInputStreams, sizeof(m_videoProcessMaxInputStreams));
	...
	hr = m_videoDevice->CheckFeatureSupport(D3D12_FEATURE_VIDEO::D3D12_FEATURE_VIDEO_PROCESS_SUPPORT, (void*)&m_videoProcessSupport, sizeof(m_videoProcessSupport));
}
...
void CVideoDevice::CreateHeapForProfile()
{
	...
	m_hr = m_videoDevice->CreateVideoDecoderHeap(&m_videoDecoderHeapDesc, IID_PPV_ARGS(&m_videoDecoderHeap));
}
...
void CVideoDevice::CreateVideoDecoder()
{
	...
	m_hr = m_videoDevice->CreateVideoDecoder(&m_videoDecoderDesc, IID_PPV_ARGS(&m_videoDecoder));
}
...
void CVideoDevice::CreateVideoProcessor()
{
	...
	m_hr = m_videoDevice->CreateVideoProcessor(0, &m_videoProcessOutputStreamDesc, m_inputStreamDescCount, &m_videoProcessInputStreamDesc, IID_PPV_ARGS(&m_videoProcessor));
}
...
char* CVideoDevice::GetProfileName(GUID g)
{
	...
	for (d = 0; d < countArray(GUIDNamesArray, GUIDNames); d++)
	{
	...
	if (d == countArray(GUIDNamesArray, GUIDNames))
	{
...
void CVideoDevice::RecordDecoder()
{
	...
	ID3D12CommandList* commandLists[] = { m_decoderCommandList.Get() };

	m_graphicsAdapter->m_commandQueue->ExecuteCommandLists(1, commandLists);
}
...
void CVideoDevice::RecordProcessor()
{
	...
	ID3D12CommandList* commandLists[] = { m_processorCommandList.Get() };
