...
CVideoDevice::CVideoDevice(CGraphicsAdapter* graphicsAdapter, CErrorLog* errorLog, GUID guid)
{
	...
	m_hr = m_graphicsAdapter->m_device->QueryInterface(IID_PPV_ARGS(&m_videoDevice));
	...
	m_hr = m_graphicsAdapter->m_device->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE::D3D12_COMMAND_LIST_TYPE_VIDEO_DECODE, IID_PPV_ARGS(&m_decoderCommandAllocator));
	m_hr = m_graphicsAdapter->m_device->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE::D3D12_COMMAND_LIST_TYPE_VIDEO_PROCESS, IID_PPV_ARGS(&m_processorCommandAllocator));

	m_hr = m_graphicsAdapter->m_device->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE::D3D12_COMMAND_LIST_TYPE_VIDEO_DECODE, m_decoderCommandAllocator.Get(), nullptr, IID_PPV_ARGS(&m_decoderCommandList));
	m_hr = m_graphicsAdapter->m_device->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE::D3D12_COMMAND_LIST_TYPE_VIDEO_PROCESS, m_processorCommandAllocator.Get(), nullptr, IID_PPV_ARGS(&m_processorCommandList));
