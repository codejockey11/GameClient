...
/*
*/
void CErrorLog::WriteXAudio2ErrorMessage(bool time, const char* format, int error)
{
	CString* errorLine = new CString(format);

	errorLine->Concat("%s %s\n");

	CXAudio2ErrorItem* xaudio2ErrorItem = m_xaudio2Error->GetError(error);

	if (xaudio2ErrorItem == nullptr)
	{
		CErrorLog::WriteError(time, errorLine->GetText(), "no name", "no msg found");
	}
	else
	{
		CErrorLog::WriteError(time, errorLine->GetText(), xaudio2ErrorItem->m_name->GetText(), xaudio2ErrorItem->m_msg->GetText());
	}
