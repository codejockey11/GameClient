...
CVisibility::CVisibility(UINT width, UINT height, UINT vertical, UINT sectorSize)
{
	...
	m_collectables = new CHeapArray(true, sizeof(CLinkList<CObject>), 2, m_sector->m_gridWidth, m_sector->m_gridHeight);

	m_statics = new CHeapArray(true, sizeof(CLinkList<CObject>), 2, m_sector->m_gridWidth, m_sector->m_gridHeight);
}
