...
CSoundDevice::CSoundDevice(CErrorLog* errorLog)
{
	...
	m_hr = CoInitializeEx(NULL, COINITBASE_MULTITHREADED);

	if (m_hr != S_OK)
	{
		m_errorLog->WriteComErrorMessage(true, "CSoundDevice::CSoundDevice::CoInitializeEx:", m_hr);
	}
	...
	m_hr = m_pCollection->GetCount(&m_count);
