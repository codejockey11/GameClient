...
CVertexBuffer::~CVertexBuffer()
{
	SAFE_DELETE_ARRAY(m_serverVertices);
...
/*
*/
void CVertexBuffer::Constructor(int32_t count, void* vertices)
{
	memset(this, 0x00, sizeof(CVertexBuffer));
...
void CVertexBuffer::CreateResource()
{
	...
	case CPipelineState::VertexType::E_VT_VERTEX2D:
	{
		...
		if (m_hr != S_OK)
		{
			m_errorLog->WriteDXGIErrorMessage(true, "CVertexBuffer::CVertexBuffer::CreateCommittedResource::CVec3f::E_VT_VERTEX2D:", m_hr);
	...
	case CPipelineState::VertexType::E_VT_VERTEXNT:
	{
		...
		if (m_hr != S_OK)
		{
			m_errorLog->WriteDXGIErrorMessage(true, "CVertexBuffer::CVertexBuffer::CreateCommittedResource::CVec3f::E_VT_VERTEXNT:", m_hr);
	...
	case CPipelineState::VertexType::E_VT_VERTEXRGBA:
	{
		...
		if (m_hr != S_OK)
		{
			m_errorLog->WriteDXGIErrorMessage(true, "CVertexBuffer::CVertexBuffer::CreateCommittedResource::CVec3f::E_VT_VERTEXRGBA:", m_hr);
	...
	case CPipelineState::VertexType::E_VT_VERTEXT:
	{
		...
		if (m_hr != S_OK)
		{
			m_errorLog->WriteDXGIErrorMessage(true, "CVertexBuffer::CVertexBuffer::CreateCommittedResource::CVec3f::E_VT_VERTEXT:", m_hr);
...
{
	m_vertexBuffer->Map(0, nullptr, &m_heapAddress);

	switch (m_type)
	{
	case CPipelineState::VertexType::E_VT_VERTEX2D:
	{
		memcpy(m_heapAddress, vertices, sizeof(CVertex2D) * m_count);

		break;
	}
	case CPipelineState::VertexType::E_VT_VERTEXNT:
	{
		memcpy(m_heapAddress, vertices, sizeof(CVertexNT) * m_count);

		break;
	}
	case CPipelineState::VertexType::E_VT_VERTEXRGBA:
	{
		memcpy(m_heapAddress, vertices, sizeof(CVertexRGBA) * m_count);

		break;
	}
	case CPipelineState::VertexType::E_VT_VERTEXT:
	{
		memcpy(m_heapAddress, vertices, sizeof(CVertexT) * m_count);

		break;
	}
	default:
	{
		m_errorLog->WriteError(true, "CVertexBuffer::LoadBuffer::Unknown VertexType\n");
...
/*
*/
void CVertexBuffer::SetCommandList(ComPtr<ID3D12GraphicsCommandList> commandList)
{
