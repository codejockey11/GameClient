...
/*
*/
CPanel::CPanel(CGraphicsAdapter* graphicsAdapter, CMouseDevice* mouseDevice, CErrorLog* errorLog, CShaderBinaryManager* shaderBinaryManager, int bundle, CImage* cursor,
	CTexture* texture, CFont* font, const char* title, CVec2f position, CVec2f size)
{
	...
	m_background = new CImage(m_graphicsAdapter, m_errorLog, shaderBinaryManager, bundle, texture, position, size);
...
void CPanel::Drag()
{
	...
	while ((m_node) && (m_node->m_object))
	{
		...
		m_button->m_image->Move(m_mouseDevice->m_hud.m_lX, m_mouseDevice->m_hud.m_lY);
	...
	while ((m_node) && (m_node->m_object))
	{
		...
		m_textInput->m_image->Move(m_mouseDevice->m_hud.m_lX, m_mouseDevice->m_hud.m_lY);
