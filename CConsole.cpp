...
/*
*/
CConsole::CConsole(CGraphicsAdapter* graphicsAdapter, CErrorLog* errorLog, CShaderBinaryManager* shaderBinaryManager, int32_t bundle, CTexture* texture, CFont* font,
...
void CConsole::DisplayText()
{
	...
	while ((m_node) && (m_node->m_object))
	{
		...
		m_font->Draw(m_listMessage->m_text, m_textPosition, m_size, m_graphicsAdapter->m_color->White);
	...
	m_font->Draw(m_message->m_text, m_textPosition, m_size, m_graphicsAdapter->m_color->White);
}
