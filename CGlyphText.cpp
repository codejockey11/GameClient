...
/*
*/
CGlyphText::CGlyphText(CGraphicsAdapter* graphicsAdapter, CErrorLog* errorLog, CShaderBinaryManager* shaderBinaryManager, CGlyphFixed* glyphFixed, int bundle,
...
/*
*/
CGlyphText::CGlyphText(CGraphicsAdapter* graphicsAdapter, CErrorLog* errorLog, CShaderBinaryManager* shaderBinaryManager, CGlyphVariable* glyphVariable, int bundle,
	const char* text, CVec2f position, float size)
{
	memset(this, 0x00, sizeof(CGlyphText));
...
void CGlyphText::AllocateResources()
{
	...
	m_shader->m_constantBuffers[0].m_values->Append(m_graphicsAdapter->m_wvpMat);
