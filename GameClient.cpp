...
ATOM                MyRegisterClass(HINSTANCE hInstance, WNDPROC wndProc, LPCWSTR className);
...
int32_t APIENTRY wWinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPWSTR lpCmdLine, _In_ int32_t nCmdShow)
{
	...
	MyRegisterClass(hInstance, WndProc, m_class);
...
BOOL InitInstance(HINSTANCE hInstance, int32_t nCmdShow)
{
	...
	const int32_t w = 1440;
	const int32_t h = 900;

	//const int32_t w = 1920;
	//const int32_t h = 1080;

	int32_t x = (GetSystemMetrics(SM_CXSCREEN) - w) / 2;
	int32_t y = (GetSystemMetrics(SM_CYSCREEN) - h) / 2;

	m_hWnd = CreateWindow(m_class, m_title, WS_TILEDWINDOW,
		x, y,
		w, h,
	...
	*/
	...
	m_graphicsAdapter = new CGraphicsAdapter(m_errorLog, m_hWnd, w, h, true, DXGI_FORMAT::DXGI_FORMAT_R8G8B8A8_UNORM, DXGI_FORMAT::DXGI_FORMAT_D32_FLOAT);
...
LRESULT CALLBACK WndProc(HWND hWnd, uint32_t message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_DESTROY:
	{
		SendMessage(m_hWnd, WM_COMMAND, IDM_EXIT, 0);
