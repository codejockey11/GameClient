...
CWavefront::CWavefront(CLocal* local, char* name)
{
	...
	m_objectScript.InitBuffer(m_filename->m_text);
	...
	m_materialFilename = new CString(m_local->m_installPath->m_text);
	
	m_materialFilename->Append(m_local->m_assetDirectory->m_text);
	m_materialFilename->Append(m_mtllib);

	m_materialScript.InitBuffer(m_materialFilename->m_text);
...
CWavefront::~CWavefront()
{
	...
	SAFE_DELETE(m_materialFilename);
