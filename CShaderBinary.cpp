...
CShaderBinary::CShaderBinary(CErrorLog* errorLog, CLocal* local, const char* name, const char* entryPoint, const char* version, int shaderModel)
{
	...
	{
		m_hr = D3DCompileFromFile(m_filename->GetWide(), nullptr, D3D_COMPILE_STANDARD_FILE_INCLUDE, m_entryPoint->m_text, m_version->m_text, flags, 0, &m_shader, &m_errors);
	...
	LPCWSTR pszArgs[] =
	{
		//L"myshader.txt",			// Optional shader source file name for error reporting
	...
	if ((m_utfErrors != nullptr) && (m_utfErrors->GetStringLength() != 0))
	{
