...
/*
*/
void CTexture::CreateFromFilename(const char* filename)
{
...
void CTexture::CreateResource(BYTE* pixels)
{
	...
	uint64_t MemToAlloc = sizeof(D3D12_PLACED_SUBRESOURCE_FOOTPRINT) + sizeof(int32_t) + sizeof(uint64_t);
	...
	m_graphicsAdapter->m_device->GetCopyableFootprints(&m_desc, 0, 1, 0, pMem, (uint32_t*)&m_height, (uint64_t*)&m_uploadData.RowPitch, &RequiredSize);
...
void CTexture::GenerateMipMaps()
{
	...
	for (int32_t topMip = 0; topMip < m_mipCount; topMip++)
	{
		...
		int32_t dstWidth = max(m_width >> (topMip + 1), 1);
		int32_t dstHeight = max(m_height >> (topMip + 1), 1);
		...
		m_commandList->SetComputeRoot32BitConstant(2, DWParam(1.0f / dstWidth).Uint32_t, 0);
		m_commandList->SetComputeRoot32BitConstant(2, DWParam(1.0f / dstHeight).Uint32_t, 1);
