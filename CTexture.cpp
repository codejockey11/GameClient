...
void CTexture::CreateResource(BYTE* pixels)
{
	...
	m_hr = m_uploadHeap->Map(0, nullptr, &m_pData);
	...
	memcpy(m_pData, m_uploadData.pData, m_uploadData.SlicePitch);
...
void CTexture::UpdateTextureResource(void* pixels)
{
	...
	m_hr = m_uploadHeap->Map(0, nullptr, &pData);
...
/*
*/
void CTexture::CreateForShadowMap()
{
	m_desc.MipLevels = 1;
	m_desc.Dimension = D3D12_RESOURCE_DIMENSION_TEXTURE2D;
	m_desc.Alignment = 0;
	m_desc.Width = m_graphicsAdapter->m_width;
	m_desc.Height = m_graphicsAdapter->m_height;
	m_desc.DepthOrArraySize = 1;
	...
	m_optClear.Format = DXGI_FORMAT::DXGI_FORMAT_D24_UNORM_S8_UINT;
	m_optClear.DepthStencil.Depth = 1.0f;
	m_optClear.DepthStencil.Stencil = 0;
	...
	if (m_hr != S_OK)
	{
		m_errorLog->WriteDXGIErrorMessage(true, "CTexture::CreateForShadowMap::CreateCommittedResource:", m_hr);
	}
}