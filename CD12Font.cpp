...
/*
*/
CD12Font::CD12Font(CErrorLog* errorLog, CVideoDevice* videoDevice, CColor* color,
	const char* face, float pitch, DWRITE_FONT_WEIGHT weight)
{
	...
	if (hr != S_OK)
	{
		m_errorLog->WriteError(true, "CD12Font::CD12Font::CreateTextFormat:%s:%s %s\n",
			face,
	...
	if (hr != S_OK)
	{
		m_errorLog->WriteError(true, "CD12Font::CD12Font::CreateSolidColorBrush:%s:%s %s\n",
			face,
			m_errorLog->GetDXGIErrorMessage(hr)->m_name->GetText(),
			m_errorLog->GetDXGIErrorMessage(hr)->m_msg->GetText());
	}
