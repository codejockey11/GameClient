...
CGraphicsAdapter::CGraphicsAdapter(CErrorLog* errorLog, HWND hWnd, int32_t width, int32_t height, bool windowed, DXGI_FORMAT backBufferFormat, DXGI_FORMAT depthStencilFormat)
{
	...
	for (int32_t i = 0; i < CCommandListBundle::E_MAX_BUNDLES; i++)
	{
...
CGraphicsAdapter::~CGraphicsAdapter()
{
	...
	for (int32_t i = 0; i < CCommandListBundle::E_MAX_BUNDLES; i++)
	{
		SAFE_DELETE(m_commandListBundle[i]);
	}
	...
	if (SUCCEEDED(DXGIGetDebugInterface1(0, IID_PPV_ARGS(&m_debug))))
	{
...
/*
*/
void CGraphicsAdapter::BundleCommandList(int32_t bundleNumber, ComPtr<ID3D12GraphicsCommandList> commandList)
{
