...
void CPlayer::Constructor(CGraphicsAdapter* graphicsAdapter, CSoundDevice* soundDevice, CSoundManager* soundManager, CErrorLog* errorLog, CLocal* local,
	...
	CConstantBuffer* overheadCameraConstantBuffer, CConstantBuffer* lightConstantBuffer, bool useOverhead, int32_t listGroup, CShadowMap* shadowMap, CServerInfo* serverInfo)
{
	memset(this, 0x00, sizeof(CPlayer));
	...
	m_camera->UpdateRotation(
		serverInfo->m_direction.m_p.x,
		serverInfo->m_direction.m_p.y,
		serverInfo->m_direction.m_p.z);

	m_camera->UpdateView();

	serverInfo->SetDirection(&m_camera->m_look);
	serverInfo->SetRight(&m_camera->m_right);
}
