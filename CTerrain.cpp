	...
	CEntity* entity, CShadowMap* shadowMap, const char* name)
{
	...
	m_indices = new CHeapArray(true, (UINT)sizeof(short) * 6, 2, m_width, m_height);

	for (short h = 0; h < m_height - 1; h++)
	{
		for (short w = 0; w < m_width - 1; w++)
		{
	...
	m_indexBuffer = new CIndexBuffer(m_graphicsAdapter, m_errorLog, m_width * m_height * 6, (void*)m_indices->m_heap);
	...
	m_overheadShader->AllocateConstantBuffers(m_shader->m_constantBufferCount);

	m_overheadShader->SetTextureCount(m_shader->m_textureCount);
	...
	m_overheadShader->CreateConstantBuffer(2, m_shader->m_constantBuffers[3].m_size, m_commandList);
	m_overheadShader->CreateConstantBuffer(3, m_shader->m_constantBuffers[2].m_size, m_commandList);
	...
	for (int i = 0; i < CTerrain::MAX_TEXTURE; i++)
	{
		...
		if (strlen(m_textureValue) > 0)
		{
			...
			m_shader->GetConstantBuffer(3)->m_values->Append(i);
			m_shader->GetConstantBuffer(3)->m_values->Append(m_scaleX);
			m_shader->GetConstantBuffer(3)->m_values->Append(m_scaleY);
			m_shader->GetConstantBuffer(3)->m_values->Append(0);

			m_overheadShader->GetConstantBuffer(3)->m_values->Append(i);
			m_overheadShader->GetConstantBuffer(3)->m_values->Append(m_scaleX);
			m_overheadShader->GetConstantBuffer(3)->m_values->Append(m_scaleY);
			m_overheadShader->GetConstantBuffer(3)->m_values->Append(0);
...
/*
*/
void CTerrain::SetLights(CLinkList<CLight>* lights)
{
	m_lights = lights;
