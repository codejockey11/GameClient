...
/*
*/
CTerrain::CTerrain(CGraphicsAdapter* graphicsAdapter, CErrorLog* errorLog, CLocal* local, CTextureManager* textureManager, CShaderBinaryManager* shaderBinaryManager, int bundle,
	CEntity* entity, CShadowMap* shadowMap, CVec3f* ambient, const char* name)
{
	...
	m_indices = new CHeapArray(true, (UINT)sizeof(short) * 6, 2, m_width, m_depth);

	for (short h = 0; h < m_depth - 1; h++)
	{
	...
	m_constantBuffer->m_values->Append(ambient);
