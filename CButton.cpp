...
/*
*/
CButton::CButton(CGraphicsAdapter* graphicsAdapter, CSoundDevice* soundDevice, CMouseDevice* mouseDevice, CErrorLog* errorLog, CShaderBinaryManager* shaderBinaryManager, int32_t bundle,
	const char* name, CFont* font, CWavLoader* soundOn, CWavLoader* soundOff, CWavLoader* soundClicked, CTexture* texture, CVec2f position, CVec2f size, BYTE audience, BYTE type)
{
	...
	m_image = new CImage(m_graphicsAdapter, errorLog, shaderBinaryManager, bundle, texture, m_position, m_size);
...
void CButton::DisplayText()
{
	...
	CVec2f position(m_position.m_p.x + (m_image->m_size.m_p.x / 2.0f) - (s.cx / 2.0f),
		m_position.m_p.y + (m_image->m_size.m_p.y / 2.0f) - (s.cy / 2.0f));
