...
CClient::CClient(CGraphicsAdapter* graphicsAdapter, CSoundDevice* soundDevice, CMouseDevice* mouseDevice, CKeyboardDevice* keyboardDevice, CErrorLog* errorLog, CLocal* local)
{
	...
	CImage* menuBackground = new CImage(m_graphicsAdapter, m_errorLog, m_shaderBinaryManager, buttonImage, CVec2f(10, 10), CVec2f(400, 400));
	...
	m_login = new CPanel(m_graphicsAdapter, m_mouseDevice, m_errorLog, m_cursor, menuBackground, m_fontArial, "Login");
...
void CClient::Connect(const char* address, const char* port)
{
	...
	if (m_gameSocket->CreateConnectSocket(address, port))
	{
		...
		m_hThread = (HANDLE)_beginthreadex(NULL, sizeof(CClient),
			&CClient::ReceiveThread,
			(void*)this,
			0,
			&m_receiveThreadId);
	}
...
void CClient::DrawLogin()
{
	...
	m_cursor->m_position = m_mouseDevice->m_hud.m_position;
...
void CClient::DrawLoading()
{
	...
	CLinkListNode<CString>* paths = shaderDirectory->m_directories->m_list;
	...
	paths = shaderDirectory->m_directories->m_list;
...
void CClient::DrawText11On12()
{
	...
	for (int i = 0; i < CServerInfo::E_MAX_CLIENTS; i++)
	{
		...
		sprintf_s(text, 132, "%02i %s %04i %06i %02i %05i %s %s %s %03i %03i %03i %06.2f %09.2f %09.2f %09.2f",
	...
	if (m_visibility != nullptr)
	{
		for (UINT pz = 0; pz < m_visibility->m_sector->m_gridHeight; pz++)
		{
			for (UINT px = 0; px < m_visibility->m_sector->m_gridWidth; px++)
			{
				...
				if (m_visibility->m_statics != nullptr)
				{
...
/*
*/
void CClient::SkipDoubleSection()
{
	while (true)
	{
...
/*
*/
void CClient::SkipSingleSection()
{
	while (true)
	{
		if (strncmp(m_mapScript.m_buffer, "}", 1) == 0)
	...
	if (!m_model[m_serverInfo->m_clientNumber].m_isInitialized)
	{
		...
		for (int i = 0; i < m_model[m_serverInfo->m_clientNumber].m_materialCount; i++)
		{
			...
			while (mesh->m_object)
			{
				...
				{
					m_model[m_serverInfo->m_clientNumber].m_shaderMaterial[mesh->m_object->m_material.m_number].m_diffuse = XMFLOAT4(1.0f, 0.0f, 0.0f, 1.0f);
					m_model[m_serverInfo->m_clientNumber].m_shaderMaterial[mesh->m_object->m_material.m_number].m_ambient = XMFLOAT4(0.5f, 0.0f, 0.0f, 1.0f);
					m_model[m_serverInfo->m_clientNumber].m_shaderMaterial[mesh->m_object->m_material.m_number].m_emissive = XMFLOAT4(1.0f, 1.0f, 1.0f, 1.0f);

					break;
				}
				case CServerInfo::E_TEAM_BLUE:
				{
					m_model[m_serverInfo->m_clientNumber].m_shaderMaterial[mesh->m_object->m_material.m_number].m_diffuse = XMFLOAT4(0.0f, 0.0f, 1.0f, 1.0f);
					m_model[m_serverInfo->m_clientNumber].m_shaderMaterial[mesh->m_object->m_material.m_number].m_ambient = XMFLOAT4(0.0f, 0.0f, 0.5f, 1.0f);
					m_model[m_serverInfo->m_clientNumber].m_shaderMaterial[mesh->m_object->m_material.m_number].m_emissive = XMFLOAT4(1.0f, 1.0f, 1.0f, 1.0f);
...
void CClient::SendActivity()
{
	...
	{
		if ((m_mouseDevice->m_keyMap[i].m_count > 0) && (m_mouseDevice->m_keyMap[i].m_network != nullptr))
		{
	...
	{
		if ((m_keyboardDevice->m_keyMap[i].m_count > 0) && (m_keyboardDevice->m_keyMap[i].m_network != nullptr))
		{
...
/*
*/
unsigned __stdcall CClient::AccountReceiveThread(void* obj)
{
	CClient* client = (CClient*)obj;

#ifdef _DEBUG
	client->m_errorLog->WriteError(true, "CClient::AccountReceiveThread:Starting\n");
#endif
	...
	while (client->m_isAccountRunning)
	{
		...
		{
			client->m_errorLog->WriteError(true, "CClient::AccountReceiveThread::Server Disconnected\n");

			client->DisconnectAccount();
		}
	}

#ifdef _DEBUG
	client->m_errorLog->WriteError(true, "CClient::AccountReceiveThread Ending\n");
#endif
...
/*
*/
unsigned __stdcall CClient::ReceiveThread(void* obj)
{
	CClient* client = (CClient*)obj;
