...
CClient::CClient(CGraphicsAdapter* graphicsAdapter, CSoundDevice* soundDevice, CMouseDevice* mouseDevice, CKeyboardDevice* keyboardDevice, CErrorLog* errorLog, CLocal* local)
{
	...
	m_buttonManager->Create(4, "Disconnect", calibri12, logon, logon, logon, buttonImage, CVec2f(10.0f, 60.0f), CVec2f(64.0f, 32.0f), CNetwork::ClientEvent::E_CE_TO_LOCAL, CNetwork::ClientEvent::E_CE_DISCONNECT);

	m_console = new CConsole(m_graphicsAdapter, m_errorLog, m_shaderBinaryManager, 5, consoleImage, consolas12, 1024, 400);

	CWavLoader* sound = m_soundManager->Get("audio/stereo/logon.wav");

	m_buttonLobby = new CButton(m_graphicsAdapter, m_soundDevice, m_mouseDevice, m_errorLog, m_shaderBinaryManager, 4, "Lobby", calibri12, sound, sound, sound, buttonImage,
	...
	m_glyphFixed = new CGlyphFixed(glyphs, 18, 16);

	glyphs = m_textureManager->Create("textures/misc/calibri.tga");

	m_glyphVariable = new CGlyphVariable(glyphs, 16);

	m_text1 = new CGlyphText(m_graphicsAdapter, m_errorLog, m_shaderBinaryManager, m_glyphFixed, 4, "This is some moving text", CVec2f(50.0f, (float)m_graphicsAdapter->m_middle.y), 16);

	m_text2 = new CGlyphText(m_graphicsAdapter, m_errorLog, m_shaderBinaryManager, m_glyphFixed, 4, "More text for reading", CVec2f(450.0f, (float)m_graphicsAdapter->m_middle.y - 100.0f), 24);

	m_text3 = new CGlyphText(m_graphicsAdapter, m_errorLog, m_shaderBinaryManager, m_glyphVariable, 4, "This is a variable sized font", CVec2f(650.0f, (float)m_graphicsAdapter->m_middle.y + 200.0f), 18);

	CTexture* cursorImage = m_textureManager->Create("textures/misc/cursor.tga");

	m_cursor = new CImage(m_graphicsAdapter, m_errorLog, m_shaderBinaryManager, 6, cursorImage, CVec2f((float)m_graphicsAdapter->m_middle.x, (float)m_graphicsAdapter->m_middle.y), CVec2f(8.0f, 32.0f));

	CButton* buttonLogon = new CButton(m_graphicsAdapter, m_soundDevice, m_mouseDevice, m_errorLog, m_shaderBinaryManager, 4, "Login", calibri12, sound, sound, sound, buttonImage,
		CVec2f(10.0f, 60.0f), CVec2f(64.0f, 32.0f), CNetwork::ClientEvent::E_CE_TO_LOCAL, CNetwork::ClientEvent::E_CE_CONNECT_ACCOUNT);

	CTexture* panelImage = m_textureManager->Create("textures/misc/black.tga");

	CTexture* textInputImage = m_textureManager->Create("textures/misc/button.tga");

	CTextInput* m_userid = new CTextInput(m_graphicsAdapter, m_mouseDevice, m_errorLog, m_shaderBinaryManager, 4, calibri12, textInputImage, CVec2f(100, 10), CVec2f(256, 14));
	CTextInput* m_password = new CTextInput(m_graphicsAdapter, m_mouseDevice, m_errorLog, m_shaderBinaryManager, 4, calibri12, textInputImage, CVec2f(100, 26), CVec2f(256, 14));

	m_login = new CPanel(m_graphicsAdapter, m_mouseDevice, m_errorLog, m_shaderBinaryManager, 4, m_cursor, panelImage, m_fontArial, "Login", CVec2f(10, 10), CVec2f(400, 400));
...
void CClient::DrawLoading()
{
	...
	m_overhead = new COverhead(m_graphicsAdapter, m_errorLog, m_local, m_shaderBinaryManager, 3, CVec2f(10.0f, 10.0f), CVec2f(256.0f, 256.0f));
	...
	CImage* loadingScreen = new CImage(m_graphicsAdapter, m_errorLog, m_shaderBinaryManager, 1, texture, CVec2f(10.0f, 10.0f), CVec2f(600.0f, 600.0f));

	CDirectoryList* shaderDirectory = new CDirectoryList();

	shaderDirectory->LoadFromDirectory(m_local->m_shaderInstallPath->m_text, ".ps");

	CListNode* pathNode = (CListNode*)shaderDirectory->m_files->m_list;

	while ((pathNode) && (pathNode->m_object))
	{
		CString* path = (CString*)pathNode->m_object;

		int32_t s = 0;

		for (int32_t i = 0; i < path->m_length; i++)
		{
			if (path->m_text[i] == '/')
			{
				s = i + 1;
		...
		m_shaderBinaryManager->Create(&path->m_text[s], CShaderBinaryManager::BinaryType::PIXEL);

		CClient::DrawLoadingScreen(loadingScreen, m_mapName->m_text, (char*)"Pixel Shaders", (char*)path->m_text);

		pathNode = pathNode->m_next;
	}
	...
	pathNode = (CListNode*)shaderDirectory->m_files->m_list;
	...
	while ((pathNode) && (pathNode->m_object))
	{
		...
		m_shaderBinaryManager->Create(&path->m_text[s], CShaderBinaryManager::BinaryType::VERTEX);

		CClient::DrawLoadingScreen(loadingScreen, m_mapName->m_text, (char*)"Vertex Shaders", (char*)path->m_text);
	...
	m_map = new CMap(m_graphicsAdapter, m_errorLog, m_local, m_soundDevice, m_wavefrontManager, m_textureManager, m_shaderBinaryManager, m_soundManager, m_mapName->m_text, m_shadowMap);
...
void CClient::DrawLogin()
{
	...
	snprintf(buffer, 32, "%f %i", m_frametime->m_frametime, m_frametime->m_rframecount);

	m_text2->SetText(buffer);
...
void CClient::DrawScene()
{
	...
	*/
	...
	{
		for (int32_t pz = 0; pz < m_map->m_visibility->m_sector->m_gridDepth; pz++)
		{
			for (int32_t px = 0; px < m_map->m_visibility->m_sector->m_gridWidth; px++)
			{
				if (m_map->m_visibility->m_collectables)
				{
					CList* collectables = (CList*)m_map->m_visibility->m_collectables->GetElement(2, px, pz);
				...
				{
					CList* statics = (CList*)m_map->m_visibility->m_statics->GetElement(2, px, pz);
			...
			for (int32_t px = 0; px < m_map->m_visibility->m_sector->m_gridWidth; px++)
			{
				...
				if ((collectables) && (collectables->m_list))
				{
					...
					while ((node) && (node->m_object))
					{
						...
						object->SetCurrentCamera(m_currentCamera);
						object->Animation(m_frametime);
						object->Update();
						object->Record();

						object->SetOverheadCamera(m_overhead->m_camera);
...
void CClient::Enter()
{
	...
	if (!m_model[m_serverInfo->m_clientNumber].m_isInitialized)
	{
		...
		m_model[m_serverInfo->m_clientNumber].Constructor(m_graphicsAdapter, m_soundDevice, m_errorLog, m_local, m_serverInfo->m_modelName, m_wavefront,
			m_textureManager, m_shaderBinaryManager, "client", true, 2, m_shadowMap, D3D12_CULL_MODE::D3D12_CULL_MODE_BACK);
		...
		m_model[m_serverInfo->m_clientNumber].SetLights(m_map->m_lights);
...
void CClient::UpdateCollectable()
{
	...
	sscanf_s((char*)m_gameNetwork->m_data, "%s %i %i %c", itemName, CKeyValue::MAX_VALUE, &px, &pz, &inLimbo, 1);

	m_collectables = (CList*)m_map->m_visibility->m_collectables->GetElement(2, px, pz);
