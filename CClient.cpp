...
{
	m_errorLog->WriteError(true, "CClient::AccountServerFull\n");
}
...
void CClient::Connect(const char* address, const char* port)
{
	...
	if (m_gameSocket->CreateConnectSocket(address, port))
	{
		...
		m_receiveThread = (HANDLE)_beginthreadex(NULL, 0,
...
void CClient::ConnectAccount(const char* address, const char* port)
{
	...
	{
		m_isAccountRunning = true;

		m_accountThread = (HANDLE)_beginthreadex(NULL, 0,
...
void CClient::Disconnect()
{
	...
	/*
		WaitForSingleObject is unecessary because _beginthreadex is a foreground thread
		and the OS will end the thread when the app finishes
	*/
...
void CClient::DrawLogin()
{
	...
	node = m_login->m_glyphTexts->m_list;

	while ((node) && (node->m_object))
	{
		CGlyphText* glyphText = (CGlyphText*)node->m_object;
...
void CClient::DrawScene()
{
	...
	for (int32_t i = 0; i < CServerInfo::E_MAX_CLIENTS; i++)
	{
		...
		if ((serverInfo->m_isRunning) || (serverInfo->m_isBot))
		{
			...
			else if (!m_player[serverInfo->m_clientNumber].m_isLoading)
			{
				m_player[serverInfo->m_clientNumber].Constructor(m_graphicsAdapter, m_soundDevice, m_soundManager, m_errorLog, m_local, m_wavefrontManager, m_textureManager,
					m_clientPipelineState, m_cameraConstantBuffer, m_overheadCameraConstantBuffer, m_map->m_lightConstantBuffer, true, 2, m_shadowMap, serverInfo);

				m_player[serverInfo->m_clientNumber].m_isLoading = true;

				_beginthreadex(NULL, 0,
					&CClient::LoadPlayerThread,
					(void*)&m_player[serverInfo->m_clientNumber],
					0,
					nullptr);
...
void CClient::InitializeLoginPanel()
{
	...
	CButton* buttonLogon = new CButton(m_graphicsAdapter, m_soundDevice, m_mouseDevice, m_errorLog, m_imageWvp, m_imagePipelineState, 1, "Login", m_calibri12,
		sound, sound, sound, buttonImage, CVec2f(106.0f, 112.0f), CVec2f(64.0f, 32.0f), CNetwork::ClientEvent::E_CE_TO_LOCAL, CNetwork::ClientEvent::E_CE_CONNECT_ACCOUNT);
...
void CClient::InitializePlayer(CServerInfo* serverInfo)
{
	...
	{
		m_player[serverInfo->m_clientNumber].SetScale(8.0f);
	}
...
void CClient::LoadEnvironment()
{
	...
	CImage* loadingScreen = new CImage(m_graphicsAdapter, m_errorLog, m_imageWvp, m_imagePipelineState, 1, texture, CVec2f(10.0f, 10.0f), CVec2f(600.0f, 600.0f));
	...
	m_graphicsAdapter->m_commandQueue->ExecuteCommandLists(1, (ID3D12CommandList* const*)loadingScreen->m_materialConstantBuffer->m_commandList.GetAddressOf());
	...
	m_loadThread = (HANDLE)_beginthreadex(NULL, 0,
	...
	while (m_isLoading)
	{
		CClient::DrawLoadingScreen(loadingScreen, m_mapName->m_text, (char*)"Loading ...", (char*)m_map->m_loadMessage->m_text);

		Sleep(100);
	}

	CClient::DrawLoadingScreen(loadingScreen, m_mapName->m_text, (char*)"Load Environment Complete", (char*)"Waiting on the server ...");

	m_graphicsAdapter->WaitForGPU();

	SAFE_DELETE(texture);
	SAFE_DELETE(loadingScreen);
	SAFE_DELETE(m_loadingText);
	SAFE_DELETE(m_mapName);

	m_errorLog->WriteError(true, "CClient::LoadEnvironment Complete\n");
...
void CClient::UpdateCollectable()
{
	...
	sscanf_s((char*)m_gameNetwork->m_data, "%s %i %i %i %c", itemName, CKeyValue::MAX_VALUE, &px, &py, &pz, &isVisible, 1);
...
/*
*/
unsigned __stdcall CClient::LoadPlayerThread(void* obj)
{
	CPlayer* player = (CPlayer*)obj;

	player->SetModel(player->m_serverInfo->m_modelName);

	player->SetPosition(&player->m_serverInfo->m_position);

	if (player->m_serverInfo->m_isBot)
	{
		player->SetScale(8.0f);
	}

	player->SetTeam(player->m_serverInfo->m_team);

	player->m_model->RecordConstantBuffers();

	player->m_model->UploadConstantBuffers();

	player->m_isInitialized = true;
