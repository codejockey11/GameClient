...
void CClient::InitializeGraphics()
{
	...
	m_fpsText = new CGlyphText(m_graphicsAdapter, m_errorLog, m_imagePipelineState, m_glyphFixed, 4, "FPS:9999", position, 14);

	position.m_p.y += m_consolas12->m_height;

	for (int32_t i = 0; i < CServerInfo::E_MAX_CLIENTS; i++)
	{
		m_scoreboardText[i] = new CGlyphText(m_graphicsAdapter, m_errorLog, m_imagePipelineState, m_glyphFixed, 4,
			"00 XXXXXXXXXX 0000 000000 00 00000 X X XXXX 000 000 000 0000.00 0000000.00 0000000.00 0000000.00 00000000000 ",
			position, 14);
...
void CClient::SendActivity()
{
	...
	int32_t sleepTime = 50 - (int32_t)(m_frametime->m_totalTime);
